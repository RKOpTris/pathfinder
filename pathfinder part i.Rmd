---
title: 'pathfinder: an algorithm for finding the most efficient path between waypoints - Part I'
author: "RKOpTris"
date: '2024-04-21'
...
```{r, run_algo_1, fig.width=12, fig.height=12}
global_vars <- new.env()
run_vars <- new.env()
...
To interpret the visualise_paths() output the first path starts as a purple arrow and gradually becomes green/yellow and gradually again becomes cyan at the finish. You might notice at "c" that it travels to "b" but then returns to "c" and so it appears as a double ended arrow (one plotted on top of the other). Note that "c" does not connect to "g" or "i" even though that path does pass by closely.
